...
def cuts(polygons, sizes, p, q):
    ...
    """
    # print( N, K ) => 19750 4
    # print(sizes.shape) => 19750
    # print(sizes) : tensor([4, 4, 4,  ..., 4, 4, 4])
    # print(p.shape, q.shape) : torch.Size([19750, 2]) torch.Size([19750, 2])
    N = polygons.shape[0]
    K = polygons.shape[1]
    ...
    v_abc = torch.stack([lines(polygons_abc[:, k, :]) for k in range(K)], dim=1)  # [N, K] torch.Size([19750, 4])
    v_bca = torch.stack([lines(polygons_bca[:, k, :]) for k in range(K)], dim=1)  # [N, K] torch.Size([19750, 4])

    # use new_polygons as stack and new_sizes as stack pointer
    # iterate and push new points
    new_polygons = torch.zeros(N, K + 1, 2).to(polygons.device).fill_(1e5)  # [N, K + 1, 2] torch.Size([19750, 5, 2])
    new_sizes = torch.zeros(N).to(polygons.device).long()  # [N,] # torch.Size([19750])
    for k in range(K):
        ...
        # 총 4번 (K = 4니까) 반복
        print(new_sizes[3781]) # 0 => 1 => 3 => 4
        new_sizes = new_sizes + mask.long().squeeze(-1)
        print(new_sizes[3781]) # 0 => 2 => 3 => 5
        
        print('-, ', (mask > 1).nonzero(as_tuple=True)[0]) # 해당되는 값 없음
        print ('--, ', (new_sizes >5).nonzero(as_tuple=True)[0])  # 해당되는 값 없음
        ...
        new_sizes = new_sizes + mask.long().squeeze(-1)# +1
        print(new_sizes[3781]) # 1 => 3 => 4 => 6
        
        print('---, ',(mask > 1).nonzero(as_tuple=True)[0]) # 해당되는 값 없음
        print ('----, ',(new_sizes >5).nonzero(as_tuple=True)[0]) 
        # 제일 마지막 반복문에서 5 이상 index 집합 리턴
        # ----,  tensor([  855,  2198,  2435,  2438,  2988,  3383,  3781,  4729,  4884,  5279,
        #  6780,  7096,  8281,  8284, 10338, 10654, 11523, 12076, 12945, 13182,
        # 13261, 14525, 14683, 15789, 16026, 16105, 16658, 16816, 16974, 17685,
        # 17764, 17843, 19739])
        
        try:
            new_polygons[torch.arange(N), new_sizes - 1, :] = push + keep 
            # new_polygons 사이즈보다 큰 값이 new_sizes안에 존재.
        except:
